...
RSpec.describe Rabarber::Core::PermissionsIntegrityChecker do
  ...
  context "when controller is specified" do
    ...
    context "when action is missing" do
      ...
      it "raises error" do
        expect { subject }.to raise_error(Rabarber::Error, "Following actions were passed to 'grant_access' method but are not defined in the controller: [{DummyAuthController=>[:index, :show]}]")
      end
    end
