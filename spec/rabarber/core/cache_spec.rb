...
RSpec.describe Rabarber::Core::Cache do
  ...
  describe ".fetch" do
    subject { described_class.fetch(roleable_id, scope) { "bar" } }
    ...
    let(:key) { described_class.prepare_key(roleable_id, scope) }
    ...
    context "when cache is enabled" do
      ...
      it "returns the cached value on subsequent calls" do
        subject
        expect(described_class.fetch(roleable_id, scope) { "baz" }).to eq("bar")
      end
      ...
      it "calls the provided block again when the cache expires" do
        ...
        expect(described_class.fetch(roleable_id, scope) { "baz" }).to eq("baz")
      end
    end
    ...
    context "when cache is disabled" do
      ...
      it "doesn't do anything when the cache expires" do
        ...
        expect(described_class.fetch(roleable_id, scope) { "baz" }).to eq("baz")
        travel_to(2.hours.from_now)
        expect(described_class.fetch(roleable_id, scope) { "bad" }).to eq("bad")
      end
    end
  end
  ...
  describe ".delete" do
    ...
    let(:key42) { described_class.prepare_key(42, scope) }
    let(:key13) { described_class.prepare_key(13, scope) }
    ...
    context "when cache is enabled" do
      ...
      it "deletes the cached value" do
        described_class.fetch(42, scope) { "bar" }
        described_class.fetch(13, scope) { "baz" }
  ...
  describe ".clear" do
    ...
    let(:keys) { roleable_ids.map { |id| described_class.prepare_key(id, scope) } }

    before do
      Rabarber.configure { |config| config.cache_enabled = true }
      roleable_ids.each { |id| described_class.fetch(id, scope) { "foo" } }
    end
  ...
  describe ".prepare_key" do
    subject { described_class.prepare_key(roleable_id, scope) }

    let(:roleable_id) { 42 }
    let(:scope) { { context_type: Project, context_id: 13 } }

    it { is_expected.to eq("rabarber:42:h/atNU+InE1Tu2Wd0H5jiw==") }
  end
end
