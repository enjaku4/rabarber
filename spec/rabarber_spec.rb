...
RSpec.describe Rabarber do
  ...
  context "when misconfigured" do
    context "with audit_trail_enabled configuration is invalid" do
      subject { described_class.configure { |config| config.audit_trail_enabled = "invalid" } }

      it "raises an error" do
        expect { subject }.to raise_error(Rabarber::ConfigurationError, "Configuration 'audit_trail_enabled' must be a Boolean")
      end
      ...
      it "uses Boolean input processor" do
        ...
        allow(Rabarber::Input::Types::Boolean).to receive(:new).with(
          "invalid", Rabarber::ConfigurationError, "Configuration 'audit_trail_enabled' must be a Boolean"
        ).and_return(double)
        subject
